    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen bg-gray-50/50">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button
          ...
          className="bg-primary hover:bg-primary-dark text-white px-5 py-2.5 rounded-[12px] shadow-sm shadow-primary/20 transition-all active:scale-[0.98] flex items-center gap-2"
        >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                <th className="text-left py-4 px-6 text-sm text-gray-900">Order</th>
                <th className="text-left py-4 px-6 text-sm text-gray-900">Title</th>
                <th className="text-left py-4 px-6 text-sm text-gray-900">Link URL</th>
                <th className="text-right py-4 px-6 text-sm text-gray-900">Actions</th>
              </tr>
            </thead>
            <tbody>
              {cards.sort((a, b) => a.order - b.order).map((card) => (
                <tr key={card.id} className="border-b border-gray-50 hover:bg-gray-50/30 transition-colors">
                  <td className="py-4 px-6">
                    <span className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-sm text-gray-700">
                      {card.order}
                    </span>
                  </td>
                  <td className="py-4 px-6">
              ...
              {cards.length === 0 && (
                <tr>
                  <td colSpan={4} className="py-12 text-center text-gray-500">
                    <p className="text-base">No resource cards found</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-[24px] shadow-xl w-full max-w-xl relative overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmit} className="px-8 py-6 space-y-5">
              <div>
                <label className="block text-sm text-gray-700 mb-1.5">Title <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  required
                  value={title}
                  ...
                  className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-primary focus:ring-2 focus:ring-primary/20 outline-none transition-all text-gray-900"
                />
              </div>

              <div>
                <label className="block text-sm text-gray-700 mb-1.5">Link URL <span className="text-red-500">*</span></label>
                <input
                  type="text"
                  required
                  value={link}
              ...
              <div>
                <label className="block text-sm text-gray-700 mb-1.5">Image Path (Optional)</label>
                <input
                  type="text"
                  value={image}
              ...
              <div>
                <label className="block text-sm text-gray-700 mb-1.5">Display Order</label>
                <input
                  type="number"
                  value={order}
              ...
              <div className="pt-4 border-t border-gray-100 flex justify-end gap-3">
                <button
                  ...
                  className="px-5 py-2.5 text-sm text-gray-600 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={saving}
                  className="px-5 py-2.5 text-sm text-white bg-primary hover:bg-primary-dark rounded-xl transition-all shadow-sm shadow-primary/20 disabled:opacity-50 flex items-center gap-2"
                >
                  {saving ? (
