    ...
    <div className="space-y-6">
      <div className="bg-white rounded-[24px] shadow-sm p-8">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={openAddModal} className="px-6 py-2.5 bg-[#007B83] text-white rounded-full shadow-md hover:bg-[#00636a] transition">
            Add Story
          </button>
        </div>
                  ...
                  return (
                    <tr key={item.id} className="hover:bg-gray-50/50 transition">
                      <td className="py-4 px-4">
                        <div className="text-[#1a3845] text-[15px]">{item.title}</div>
                        {item.section && item.section !== 'Stories' && (
                          <div className="text-gray-500 text-[12px]">{item.section}</div>
                        )}
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className={`text-[12px] px-3 py-1 rounded-full  ${hasImage ? 'bg-[#e6f4f1] text-[#007B83]' : 'bg-red-50 text-red-500'}`}>
                          {hasImage ? 'Yes' : 'No'}
                        </span>
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className={`text-[12px] px-3 py-1 rounded-full  ${hasFile ? 'bg-[#e6f4f1] text-[#007B83]' : 'bg-red-50 text-red-500'}`}>
                          {hasFile ? 'Yes' : 'No'}
                        </span>
                      </td>
                      <td className="py-4 px-4 text-center">
                        <span className={`text-[12px]  ${item.is_active ? 'text-[#007B83]' : 'text-red-500'}`}>
                          {item.is_active ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="py-4 px-4 text-right space-x-2">
                        <a href={`${process.env.NEXT_PUBLIC_FRONTEND_URL || 'http://localhost:3000'}/missionary-stories/${item.slug || item.id}`} target="_blank" rel="noreferrer" className="inline-block px-4 py-1.5 bg-[#e6f4f1] text-[#007B83] rounded-full text-[13px] hover:bg-[#cde9e3] transition">
                          View
                        </a>
                        <button onClick={() => openEditModal(item)} className="px-4 py-1.5 bg-[#007B83] text-white rounded-full text-[13px] hover:bg-[#00636a] transition">
                          Edit
                        </button>
                        <button onClick={() => handleToggleStatus(item)} className="px-4 py-1.5 bg-[#f0f7f7] text-[#007B83] rounded-full text-[13px] hover:bg-[#e0f0f0] transition">
                          {item.is_active ? 'Inactive' : 'Active'}
                        </button>
                        <button onClick={() => confirmDelete(item.id)} className="px-4 py-1.5 bg-[#b22222] text-white rounded-full text-[13px] hover:bg-[#8e1b1b] transition">
                          Delete
                        </button>
                      </td>
                    </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[24px] shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">Story Title</label>
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#007B83] outline-none transition text-gray-900" />
              </div>

              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">Subtitle (Optional)</label>
                <input type="text" value={formData.subtitle} onChange={e => setFormData({...formData, subtitle: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#007B83] outline-none transition text-gray-900" />
              </div>
              
              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">Image URL</label>
                <input type="text" value={formData.image_url} onChange={e => setFormData({...formData, image_url: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#007B83] outline-none transition text-gray-900 mb-2" />
                <label className="px-4 py-2 bg-white border border-[#007B83] text-[#007B83] rounded-full cursor-pointer hover:bg-[#f0f7f7] transition text-[12px] inline-block">
                  Upload Cover Image
              ...
              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">File/Document URL</label>
                <input type="text" value={formData.file_url} onChange={e => setFormData({...formData, file_url: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#007B83] outline-none transition text-gray-900 mb-2" />
              </div>

              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-[16px] border border-gray-100">
                <span className="text-[14px] text-gray-800">Set as Active</span>
              ...
              <div className="pt-4 flex justify-end gap-3 sticky bottom-0 bg-white pb-2 border-t border-gray-100 mt-4">
                <button type="button" onClick={() => setShowModal(false)} className="px-6 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-full hover:bg-gray-50 transition">Cancel</button>
                <button type="submit" className="px-8 py-2.5 bg-[#007B83] text-white rounded-full shadow-md hover:bg-[#00636a] transition">{isEditing ? 'Save Changes' : 'Add Story'}</button>
              </div>
            </form>
          </div>
        </div>
