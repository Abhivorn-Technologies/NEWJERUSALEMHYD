  ...
  return (
    <div className="space-y-6 text-[#1A4B5C]">
      ...
      <div className="bg-white rounded-[20px] shadow-sm py-5 px-8 flex items-center justify-between">
        ...
        <span className="text-[#0B7A8A] text-sm">admin</span>
      </div>
      ...
      <div className="bg-white rounded-[30px] shadow-sm p-8 pb-16">
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
          ...
          <button
            ...
            className="px-6 py-2.5 bg-[#0B7A8A] text-white rounded-full hover:bg-[#09626e] transition-colors shadow-sm text-sm"
          >
        ...
        {showAddForm && (
          <div className="bg-[#EAF5F8] p-6 rounded-2xl mb-8 border border-[#BDE0E8]">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                <label className="block text-sm text-[#0B7A8A] mb-1">Title</label>
                <input
                  type="text"
                  required
                  value={title}
              ...
              <div>
                <label className="block text-sm text-[#0B7A8A] mb-1">Month & Year / Version</label>
                <input
                  type="text"
                  required
                  value={monthYear}
              ...
              <div>
                <label className="block text-sm text-[#0B7A8A] mb-1">Language</label>
                <select
                  value={language}
              ...
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                  <label className="block text-sm text-[#0B7A8A] mb-1">Cover Image</label>
                  <input
                    type="file"
                    required={!isEditing}
                    accept="image/*"
                    onChange={(e) => setCoverImage(e.target.files?.[0] || null)}
                    className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file: file:bg-white file:text-[#0B7A8A] hover:file:bg-gray-50"
                  />
                  {isEditing && <p className="text-xs text-gray-500 mt-1">Leave blank to keep current cover.</p>}
                </div>
                <div>
                  <label className="block text-sm text-[#0B7A8A] mb-1">Magazine PDF</label>
                  <input
                    type="file"
                    required={!isEditing}
              ...
              <div className="md:col-span-2 pt-2">
                <button
                  type="submit"
                  disabled={uploading}
                  className="px-6 py-2.5 bg-[#871936] text-white rounded-full hover:bg-[#6c142b] transition disabled:opacity-50 disabled:cursor-not-allowed shadow-sm text-sm"
                >
        ...
        {/* Table */}
        {loading ? (
          <div className="py-12 text-center text-[#0B7A8A]">Loading magazines...</div>
        ) : filteredMagazines.length === 0 ? (
          <div className="py-12 text-center text-gray-500">No magazines found.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider pl-2">Title</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Language</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">File Size</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Downloads</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Status</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Actions</th>
                </tr>
              </thead>
                ...
                {filteredMagazines.map((mag, i) => (
                  <tr key={mag.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                    <td className="py-5 pl-2">
                      <div className="text-[#053245] text-[15px]">{mag.title}</div>
                      <div className="text-[12px] text-gray-400 mt-1">{mag.month_year || 'v2'}</div>
                    </td>
                    <td className="py-5 text-gray-600 text-[14px]">{mag.language}</td>
                    <td className="py-5 text-[#053245] text-[13px]">15.35 KB</td>
                    <td className="py-5 text-[#053245] text-[14px]">2</td>
                    <td className="py-5">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px]  ${mag.is_active !== false ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-[#FFEBEE] text-[#C62828]'}`}>
                    ...
                    <td className="py-5 space-x-2">
                      <button 
                        ...
                        className="px-4 py-1.5 bg-[#0B7A8A] text-white text-xs rounded-full hover:bg-[#09626e] transition-colors"
                      >
                        Edit
                      </button>
                      <button 
                        onClick={() => handleToggleStatus(mag)}
                        className="px-4 py-1.5 bg-[#EAF5F8] text-[#0B7A8A] text-xs rounded-full hover:bg-[#D5EBEF] transition-colors"
                      >
                        {mag.is_active !== false ? 'Deactivate' : 'Activate'}
                      </button>
                      <button
                        onClick={() => confirmDelete(mag.id)}
                        className="px-4 py-1.5 bg-[#B32625] text-white text-xs rounded-full hover:bg-[#921f1e] transition-colors shadow-sm"
                      >
                        Delete
                      </button>
                    </td>
