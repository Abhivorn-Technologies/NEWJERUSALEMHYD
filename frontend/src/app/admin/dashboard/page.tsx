...
export default function DashboardPage() {
  ...
  if (loading) {
    return <div className="animate-pulse p-4 text-[#1a3845] text-center mt-12 text-lg">Loading dashboard...</div>;
  }

  if (error || !stats) {
    return <div className="p-4 text-red-500 text-center mt-12 text-lg">Failed to load dashboard data. Ensure the backend is running and you are logged in.</div>;
  }
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden animate-fade-up" style={{ animationDelay: '800ms' }}>
          <div className="px-6 py-5 border-b border-gray-100 bg-white flex justify-between items-center">
            ...
            <Link href="/admin/dashboard/content?section=Old Testament" className="bg-[#128a95] hover:bg-[#0f717a] text-white text-[13px] px-5 py-2 rounded-full transition-colors">
              View All
            </Link>
          </div>
          <div className="p-6 bg-[#f8fafc]">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {storiesSections.map(sec => {
                ...
                return (
                  <Link href={`/admin/dashboard/content?section=${sec}`} key={sec} className="bg-white rounded-[16px] p-5 shadow-sm border border-gray-100 hover:shadow-md hover:border-[#128a95]/30 transition group flex flex-col justify-between">
                    ...
                    <span className="text-[12px] text-gray-400">{counts.active} active / {counts.total} total</span>
                  </Link>
                );
              })}
            </div>
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden animate-fade-up" style={{ animationDelay: '900ms' }}>
          <div className="px-6 py-5 border-b border-gray-100 bg-white flex justify-between items-center">
            ...
            <Link href="/admin/dashboard/resources?category=bible-infographics" className="bg-[#128a95] hover:bg-[#0f717a] text-white text-[13px] px-5 py-2 rounded-full transition-colors">
              View All
            </Link>
          </div>
          <div className="p-6 bg-[#f8fafc]">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {resourcesSections.map(sec => {
                ...
                return (
                  <Link 
                    href={sec === 'Missionary Stories' ? '/admin/dashboard/missionary-stories' : `/admin/dashboard/resources?category=${titleMap[sec] || sec}`} 
                    key={sec} 
          ...
          return (
            <div key={lang} className="bg-white rounded-[24px] shadow-sm border border-gray-100 p-8 flex flex-col sm:flex-row items-center justify-between text-center sm:text-left animate-fade-up" style={{ animationDelay: `${1000 + (idx * 100)}ms` }}>
              <div>
                ...
                <p className="text-[14px] text-gray-500 mt-1">{lstat.active} active / {lstat.inactive} inactive</p>
              </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[400px] animate-fade-up" style={{ animationDelay: '1300ms' }}>
          <div className="px-6 py-5 border-b border-gray-100 bg-white flex justify-between items-center">
            ...
            <Link href="/admin/dashboard/songs/categories" className="text-[13px] text-[#128a95] hover:underline">Manage Categories</Link>
          </div>
            ...
            <table className="w-full text-left text-[14px]">
              <thead className="bg-white sticky top-0 border-b border-gray-100 z-10">
                <tr>
                  ...
                  <th className="px-4 py-3 text-gray-500 text-center">Total Songs</th>
                  <th className="px-4 py-3 text-gray-500 text-right">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {stats.category_breakdown.map((cat: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-[#1a3845]">{cat.name}</td>
                    <td className="px-4 py-3 text-center text-gray-600">{cat.total}</td>
                    <td className="px-4 py-3 text-right">
                      {cat.active > 0 ? (
                        <span className="text-[#10b981] bg-[#10b981]/10 px-2 py-1 rounded-md text-[12px]">Active</span>
                      ) : (
                        <span className="text-[#ef4444] bg-[#ef4444]/10 px-2 py-1 rounded-md text-[12px]">Inactive</span>
                      )}
                    </td>
                  </tr>
                ))}
        ...
        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 overflow-hidden flex flex-col h-[400px] animate-fade-up" style={{ animationDelay: '1400ms' }}>
          ...
          <div className="overflow-y-auto flex-1 p-2">
            <table className="w-full text-left text-[14px]">
              <thead className="bg-white sticky top-0 border-b border-gray-100 z-10">
                <tr>
                  <th className="px-4 py-3 text-gray-500">Language</th>
                  <th className="px-4 py-3 text-gray-500 text-center">Letter</th>
                  <th className="px-4 py-3 text-gray-500 text-right">Total Songs</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50">
                {stats.letter_breakdown.map((l: any, i: number) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-[#1a3845]">{l.language}</td>
                    <td className="px-4 py-3 text-center text-[#128a95]">{l.first_letter || '#'}</td>
                    <td className="px-4 py-3 text-right text-gray-600">{l.total}</td>
                  </tr>
                ))}
              </tbody>
            </table>
    ...
    >
      <div className="flex justify-between items-start relative z-10">
        <div>
          ...
          <p className="text-[13px] text-white/90">{sub}</p>
        </div>
