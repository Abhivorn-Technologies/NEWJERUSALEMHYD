  ...
  return (
    <div className="space-y-6 text-[#1A4B5C]">
      ...
      <div className="bg-white rounded-[30px] shadow-sm p-8 pb-16">
        ...
        {showAddForm && (
          <div className="bg-[#EAF5F8] p-6 rounded-2xl mb-8 border border-[#BDE0E8]">
            ...
            <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="md:col-span-2">
                <label className="block text-sm text-[#0B7A8A] mb-1">Subtitle / Scripture Reference</label>
                <input
                  type="text"
                  value={subtitle}
              ...
              <div>
                <label className="block text-sm text-[#0B7A8A] mb-1">Section</label>
                <select
                  value={section}
              ...
              <div>
                <label className="block text-sm text-[#0B7A8A] mb-1">PDF Link (URL)</label>
                <input
                  type="text"
                  value={pdfLink}
        ...
        {/* Table */}
        {loading ? (
          <div className="py-12 text-center text-[#0B7A8A]">Loading items...</div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider pl-2 w-12">#</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Title</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Section</th>
                  <th className="pb-4 text-[11px] text-[#053245] uppercase tracking-wider">Links</th>
                ...
                {filteredItems.map((item, i) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="py-5 pl-2 text-gray-500 text-[14px]">{(i + 1) * 10}</td>
                      <td className="py-5 text-[#053245] text-[14px] max-w-[300px] pr-4">{item.title}</td>
                      <td className="py-5 text-gray-600 text-[13px]">{sectionsMap[item.section] || item.section}</td>
                      <td className="py-5">
                        {pdfLinkStr ? (
                          <a href={pdfLinkStr} target="_blank" rel="noreferrer" className="text-[#053245] underline text-[13px] hover:text-[#0B7A8A]">
                            PDF
                          </a>
                        ) : (
                      ...
                      <td className="py-5">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px]  ${item.is_active ? 'bg-[#E8F5E9] text-[#2E7D32]' : 'bg-[#FFEBEE] text-[#C62828]'}`}>
                          {item.is_active ? 'Active' : 'Inactive'}
                        </span>
                      </td>
                      <td className="py-5 space-x-2 whitespace-nowrap">
                        <button onClick={() => handleOpenEdit(item)} className="px-4 py-1.5 bg-[#0B7A8A] text-white text-xs rounded-full hover:bg-[#09626e] transition-colors">
                          Edit
                        </button>
                        <button onClick={() => handleToggleStatus(item)} className="px-4 py-1.5 bg-[#EAF5F8] text-[#0B7A8A] text-xs rounded-full hover:bg-[#D5EBEF] transition-colors">
...
export default function ContentItemsPage() {
  return (
    <Suspense fallback={<div className="py-12 text-center text-[#0B7A8A]">Loading content...</div>}>
