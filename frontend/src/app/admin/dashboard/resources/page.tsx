...
const SLUG_TO_NAME: Record<string, string> = {
  'bible-infographics': 'Bible Infographics',
  'bible-maps': 'Bible Maps',
  'bible-genealogies': 'Bible Genealogies',
...
export default function BibleResourcesPage() {
  ...
  const categoryParam = categorySlug && SLUG_TO_NAME[categorySlug] ? SLUG_TO_NAME[categorySlug] : null;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-[24px] px-8 py-5 flex items-center justify-between shadow-sm border border-gray-100">
        ...
        <div className="text-[#D04A73] text-sm cursor-pointer hover:underline">
          admin
        </div>
      </div>
      ...
      <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 p-8">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={openAddModal} className="px-6 py-2.5 bg-[#4D1C2C] text-white rounded-full shadow-md hover:bg-[#3a1521] transition">
            Add Resource
          </button>
        </div>
              ...
              {loading ? (
                ...
                filteredItems.map(item => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition">
                    <td className="py-4 px-2 text-[#1a3845]">{item.title}</td>
                      ...
                      {item.links && Array.isArray(item.links) && item.links.length > 0 && (
                        <div className="flex flex-wrap gap-1">
                          {item.links.map((l: any, i: number) => (
                            <span key={i} className="text-[11px] bg-gray-100 px-2 py-0.5 rounded text-gray-600 border border-gray-200">
                              {l.text}
                            </span>
                          ))}
                        </div>
                      )}
                      {(item.image_url || item.cover_image) && <span className="text-[11px] bg-blue-50 text-blue-600 px-2 py-0.5 rounded border border-blue-100 inline-block mt-1">Image</span>}
                    </td>
                    <td className="py-4 px-2">
                      <span className={`text-[13px]  ${item.is_active ? 'text-[#D04A73]' : 'text-gray-400'}`}>
                        {item.is_active ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className="py-4 px-2 flex flex-wrap gap-2">
                      <button onClick={() => openEditModal(item)} className="px-4 py-1.5 bg-[#4D1C2C] text-white rounded-full text-[13px] hover:bg-[#3a1521] transition">Edit</button>
                      <button onClick={() => handleToggleStatus(item)} className="px-4 py-1.5 bg-[#FADADD]/30 text-[#4D1C2C] rounded-full text-[13px] hover:bg-[#FADADD]/60 transition">
                        {item.is_active ? 'Inactive' : 'Active'}
                      </button>
                      <button onClick={() => confirmDelete(item.id)} className="px-4 py-1.5 bg-[#b22222] text-white rounded-full text-[13px] hover:bg-[#8e1b1b] transition">Delete</button>
                    </td>
                  </tr>
                ))
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[24px] shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">Resource Title</label>
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#D04A73] outline-none transition text-gray-900" />
              </div>
              
              <div className="grid grid-cols-2 gap-6">
                <div>
                  <label className="block text-[14px] text-gray-700 mb-1.5">Page Category</label>
                  <select value={formData.page_category} onChange={e => handlePageChange(e.target.value)} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#D04A73] outline-none transition text-gray-900">
                    {PAGE_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-[14px] text-gray-700 mb-1.5">Section</label>
                  <select value={formData.section} onChange={e => setFormData({...formData, section: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#D04A73] outline-none transition text-gray-900">
              ...
              <div>
                <label className="block text-[14px] text-gray-700 mb-1.5">Cover Image URL (Optional)</label>
                <input type="text" value={formData.image_url} onChange={e => setFormData({...formData, image_url: e.target.value})} placeholder="/wp-content/uploads/..." className="w-full p-3 bg-gray-50 border border-gray-200 rounded-[12px] focus:bg-white focus:border-[#D04A73] outline-none transition text-gray-900 mb-3" />
                
                <label className="block text-[14px] text-gray-700 mb-1.5">Or Upload Direct Cover Image</label>
                <div className="flex items-center gap-4">
                  <label className="px-5 py-2.5 bg-white border border-[#D04A73] text-[#D04A73] rounded-full cursor-pointer hover:bg-[#FADADD]/30 transition text-[13px]">
                    Choose File
                    <input 
                      type="file" 
                      className="hidden" 
                  ...
                  {formData.cover_image && <span className="text-[13px] text-gray-600">{formData.cover_image.name}</span>}
                </div>
              </div>

              <div className="bg-gray-50 p-4 rounded-[16px] border border-gray-100">
                <div className="flex justify-between items-center mb-4">
                  <label className="block text-[14px] text-gray-800">Resource Links</label>
                  <button type="button" onClick={addLinkField} className="flex items-center gap-1 text-[13px] text-[#D04A73] bg-white px-3 py-1.5 rounded-full border border-gray-200 hover:bg-[#FADADD]/20 transition">
                  ...
                  {formData.links.map((link, index) => (
                    <div key={index} className="flex gap-3 items-center">
                      <input 
                        ...
                        className="w-1/3 p-2.5 bg-white border border-gray-200 rounded-[8px] focus:border-[#D04A73] outline-none text-sm text-gray-900" 
                      />
                      <input 
                        type="text" 
                ...
                <p className="text-[12px] text-gray-500 mt-3">Add multiple links like PDF, JPEG, Color, or Black & White.</p>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-[16px] border border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[15px] text-gray-800">Set as Active</span>
                </div>
                <span className="text-[12px] text-gray-500">Inactive resources will be hidden from the website.</span>
              </div>

              <div className="pt-4 flex justify-end gap-3 border-t border-gray-100 mt-6 sticky bottom-0 bg-white pb-2">
                <button type="button" onClick={() => setShowModal(false)} className="px-6 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-full hover:bg-gray-50 transition">
                  Cancel
                </button>
                <button type="submit" className="px-8 py-2.5 bg-[#4D1C2C] text-white rounded-full shadow-md hover:bg-[#3a1521] transition">
