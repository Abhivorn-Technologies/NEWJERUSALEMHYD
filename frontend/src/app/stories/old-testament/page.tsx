...
export default async function OldTestamentStoriesPage() {
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
  let storiesList = [];
  try {
    const res = await fetch(`${API_URL}/api/content-items/?page_category=Bible+Stories+%26+Activities&section=Old+Testament`, { cache: 'no-store' });
    if (res.ok) {
