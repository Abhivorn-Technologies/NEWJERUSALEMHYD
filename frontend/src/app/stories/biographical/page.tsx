...
export default async function StoriesPage() {
  ...
  let storiesList = [];
  try {
    const res = await fetch(`${API_URL}/content-items/?page_category=Bible+Stories+%26+Activities&section=Biographical`, { cache: 'no-store' });
    if (res.ok) {
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-0 mt-4">
            <div className="bg-[#FF99BE] px-4 sm:px-8 py-5 text-white flex flex-row items-center justify-between gap-4 rounded-t-2xl sm:rounded-2xl shadow-md mb-6">
              <div>
                ...
                <p className="text-white/80 text-sm mt-1 max-w-md">Review the biographical details, timelines, and life stories of key biblical figures.</p>
              </div>
              <img 
