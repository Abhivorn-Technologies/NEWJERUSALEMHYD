...
export default async function PreSchoolStoriesPage() {
  const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:8000';
  let storiesList = [];
  try {
    const res = await fetch(`${API_URL}/api/content-items/?page_category=Bible+Stories+%26+Activities&section=Pre+School`, { cache: 'no-store' });
    if (res.ok) {
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-0 mb-10">
            <div className="bg-[#FF99BE] px-4 sm:px-8 py-6 text-white flex flex-row items-center justify-between gap-4 rounded-t-2xl sm:rounded-2xl shadow-md mb-6">
              <div>
                ...
                <p className="text-white/80 text-sm mt-1 max-w-md">Simple, interactive stories tailored for preschool children.</p>
              </div>
              <img 
