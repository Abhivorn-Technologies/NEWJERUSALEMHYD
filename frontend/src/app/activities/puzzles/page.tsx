...
export default async function PuzzlesPage() {
  ...
  try {
    const otRes = await fetch(`${API_URL}/content-items/?page_category=Activities&section=Puzzles+OT`, { cache: 'no-store' });
    if (otRes.ok) otStories = await otRes.json();

    const ntRes = await fetch(`${API_URL}/content-items/?page_category=Activities&section=Puzzles+NT`, { cache: 'no-store' });
