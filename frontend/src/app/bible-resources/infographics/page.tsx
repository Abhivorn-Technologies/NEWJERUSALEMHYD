...
async function fetchBibleInfographics() {
  try {
    const res = await fetch('http://127.0.0.1:8000/api/content-items/?page_category=Bible+Infographics', {
      cache: 'no-store'
    });
    if (!res.ok) return [];
    return await res.json();
  } catch (error) {
    console.error('Error fetching Bible Infographics:', error);
    return [];
  }
}

export default async function BibleInfographicsPage() {
  const allItems = await fetchBibleInfographics();
    ...
    <div className="min-h-screen bg-[#e8f4f8] py-16 px-6">
      <div className="max-w-5xl mx-auto">
        <div className="text-center mb-14">
          ...
          <h1 className="text-4xl md:text-5xl font-extrabold text-[#1f4251] mb-4">BIBLE INFOGRAPHICS</h1>
        ...
        <div className="space-y-14">
          ...
          {activeItems.length === 0 && (
            <p className="text-center text-gray-500">No infographics found.</p>
          )}
        </div>
      </div>
    </div>
